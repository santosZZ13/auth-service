...
import org.gateway.aiscore.exception.AlreadyJobException;
...
public class CustomTaskScheduler extends ThreadPoolTaskScheduler {
	...
	public void scheduleWithParams(Runnable task, Object type, @NotNull Job job)  {

		Set<String> jobNames = getJobNames();

		if (jobNames.contains(job.getJobName())) {
			throw new AlreadyJobException("Job with name " + job.getJobName() + " already exists");
		}
