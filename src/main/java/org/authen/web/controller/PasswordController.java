...
public class PasswordController {
	...
	public ResponseEntity<GenericResponseSuccessWrapper> resetPassword(@RequestParam("email") String email, HttpServletRequest request) {
		log.info("#Resetting password for user with email: {}", email);
		return passwordService.resetPassword(email, request);
	}

	@PostMapping(value = "/password/update")
	public ResponseEntity<GenericResponseSuccessWrapper> changeUserPassword(Locale locale, @RequestBody UpdatePasswordDTO updatePasswordDTO) {
